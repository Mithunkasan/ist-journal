...
const GuestEditorsManagement = () => {
  const router = useRouter();
  const [loading, setLoading] = useState(true);
  const [guestEditors, setGuestEditors] = useState<any[]>([]);
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(5);

  const fetchGuestEditors = async () => {
    setLoading(true);
    try {
      const response = await axios.get("/api/get-guest-editor");
      setGuestEditors(response.data);
    } catch (error) {
      console.error("Failed to fetch guest-editors", error);
    } finally {
      setLoading(false);
    }
  };
...
export default GuestEditorsManagement;
