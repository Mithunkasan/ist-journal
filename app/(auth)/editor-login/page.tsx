...
import { DEFAULT_EDITOR_REDIRECT } from "@/routes";
...
const EditorLoginPageContent = () => {
  ...
  const getRedirectPath = useCallback(() => {
    return searchParams.get("callbackUrl") || DEFAULT_EDITOR_REDIRECT;
  }, [searchParams]);

  useEffect(() => {
    if (status === "authenticated" && session?.user?.role === "EDITOR") {
      router.push(getRedirectPath());
    } else if (status === "authenticated" && session?.user?.role !== "EDITOR") {
      setLoginError("Unauthorized. This portal is for Chief Editors only.");
    }
        ...
        <Container maxWidth="sm">
          ...
          >
            ...
            <Typography variant="h6" sx={{ mb: 4, color: '#666', textAlign: 'center' }}>
              Secure Gateway for Chief Editors
            </Typography>
