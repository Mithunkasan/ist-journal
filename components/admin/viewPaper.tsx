...
export default function ViewPaper({ modalOpen, setModalOpen, data }: any) {
  ...
  const paperUrl = data?.[0]?.paperUrl;
  const absoluteUrl = typeof window !== "undefined" && paperUrl && paperUrl.startsWith("/")
    ? `${window.location.origin}${paperUrl}`
    : paperUrl;
